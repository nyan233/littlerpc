//go:build !go1.18 && !go1.19 && !go.1.20
...
type Client struct {
	...
	// 使用到的操作均是线程安全的
	rbMu        sync.RWMutex
	readyBuffer map[uint64]protocol.Message
...
func addReadyBuffer(c *Client, msgId uint64, msg protocol.Message) {
	c.rbMu.RLock()
	defer c.rbMu.RUnlock()
	c.readyBuffer[msgId] = msg
}

func loadReadyBuffer(c *Client, msgId uint64) protocol.Message {
	c.rbMu.Lock()
	defer c.rbMu.Unlock()
	return c.readyBuffer[msgId]
}
...
func loadElems(c *Client, instanceName string) (common.ElemMeta, bool) {
	tmp, ok := c.elems.Load(instanceName)
	return tmp.(common.ElemMeta), ok
}
...
func loadCallBack(c *Client, processName string) (func(rep []interface{}, err error), bool) {
	tmp, ok := c.callBacks.Load(processName)
	return tmp.(func(rep []interface{}, err error)), ok
}
